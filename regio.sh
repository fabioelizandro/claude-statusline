...
# Uses 256-colour ANSI with a palette deep enough to read on light and dark terminal themes.
input=$(cat)
j() { echo "$input" | jq -r "$1"; }
...
R=$'\e[0m'; BOLD=$'\e[1m'
PINK=$(c 198); CYAN=$(c 37); PURPLE=$(c 129); YELLOW=$(c 214); GREEN=$(c 34); GREY=$(c 244)
sep=" ${PURPLE}//${R} "
